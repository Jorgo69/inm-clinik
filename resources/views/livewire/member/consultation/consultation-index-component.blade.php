<div>
    ...
    <div class="mt-6 md:flex md:items-center md:justify-between">
        <div class="inline-flex overflow-hidden bg-white border divide-x rounded-lg dark:bg-gray-900 rtl:flex-row-reverse dark:border-gray-700 dark:divide-gray-700">
            <button 
                wire:click="$set('sortOrder', 'all')" 
                class="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 bg-gray-100 sm:text-sm dark:bg-gray-800 dark:text-gray-300">
                Tout voir
            </button>
            
            <button 
                wire:click="$set('sortOrder', 'recent')" 
        ...
        <div class="relative flex items-center mt-4 md:mt-0">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Rechercher une consultation" class="block w-full py-1.5 pr-5 text-gray-700 bg-white border border-gray-200 rounded-lg md:w-80 placeholder-gray-400/70 pl-11 rtl:pr-11 rtl:pl-5 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40">
        </div>
    </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                            ...
                            <tr>
                                <td class="px-12 py-4 text-sm font-medium whitespace-nowrap">
                                    <div class="inline px-3 py-1 text-sm font-normal capitalize rounded-full text-emerald-500 gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                                        {{\Carbon\Carbon::parse($consultation->created_at)->translatedFormat('l jS F Y')}}
                                    </div>
                                </td>
                                  <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                      <div>
                                          <h2 class="font-medium text-gray-800 dark:text-white ">{{$consultation->patient->name .' '. $consultation->patient->firstname}}</h2>
                                          <p class="text-sm font-normal text-gray-600 dark:text-gray-400">{{$consultation->patient->email}}</p>
                                      </div>
                                  </td>
                                  <td class="px-12 py-4 text-sm font-medium whitespace-nowrap">
                                  ...
                                  <td class="px-4 py-4 text-sm whitespace-nowrap">
                                      <div class="flex items-center">
                                        <span class="size-12 uppercase text-center flex items-center justify-center text-xl tracking-tighter font-bold bg-gray-200 hover:ring-blue-500 animate rounded-full dark:bg-white ring-4 ring-gray-300 dark:ring-white shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40">
                                            ...
                                            {{substr($consultation->patient->firstname, 0, 1)}}
                                        </span>
                                      </div>
                                  </td>
